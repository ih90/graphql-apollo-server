...
export const getJobBenefitsByIds = async (ids) => {
  ...
  const query = `SELECT *, UNIX_TIMESTAMP(createdAt) as createdAt, UNIX_TIMESTAMP(updatedAt) as updatedAt from jobBenefits WHERE id IN (${ids})`;
...
export const getJobBenefits = async () => {
  const connection = await getConnection();
  const query = 'SELECT *, UNIX_TIMESTAMP(createdAt) as createdAt, UNIX_TIMESTAMP(updatedAt) as updatedAt from jobBenefits';
...
export const getJobBenefitsByJobId = async (jobId) => {
  const connection = await getConnection();
  const query = 'SELECT *, UNIX_TIMESTAMP(createdAt) as createdAt, UNIX_TIMESTAMP(updatedAt) as updatedAt from jobBenefits where jobId = :jobId';
...
export const createJobBenefit = async (args) => {
  ...
  INSERT jobBenefits (createdAt, ${Object.keys(args)
    .map((c) => `\`${c}\``)
    .join(', ')})
