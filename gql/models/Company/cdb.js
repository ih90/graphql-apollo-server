...
export const getCompaniesByIds = async (ids) => {
  ...
  const query = `SELECT *, UNIX_TIMESTAMP(createdAt) as createdAt, UNIX_TIMESTAMP(updatedAt) as updatedAt from companies WHERE id IN (${ids})`;
...
export const getCompanies = async () => {
  const connection = await getConnection();
  const query = 'SELECT *, UNIX_TIMESTAMP(createdAt) as createdAt, UNIX_TIMESTAMP(updatedAt) as updatedAt from companies';
...
export const createCompany = async (args) => {
  ...
  INSERT companies (createdAt, ${Object.keys(args)
    .map((c) => `\`${c}\``)
    .join(', ')})
